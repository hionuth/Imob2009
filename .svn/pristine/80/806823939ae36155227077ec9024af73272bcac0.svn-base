...
if ((isset($_POST['submit']))||(isset($_POST['actualizare']))) {
	...
	$syncList=array('ExportPitagora','ExportImobiliare','ExportRoImobile','ExportRomimo', 'ExportMC', 'ExportImopedia','ExportPB');
	...
	foreach($postlist as $variable) {
		...
		if (substr($variable,0,2)=="Of") {
			if ((in_array($atribut, $syncList))||($atribut=="OfertaWeb")){
				$syncsite=$atribut;
	...
	foreach ($syncList as $sync) {
		if ((!isset(${"Of".$sync}))||($oferta->Stare!="de actualitate")) {
				$oferta->{$sync}=($oferta->{$sync}>1 ? 3 : 0);
		}
	}
	
	if ((!isset($OfOfertaWeb))||($oferta->Stare=="stand by")) {
		$oferta->OfertaWeb=$oferta->OfertaWeb>1 ? 3 : 0;
	}
	...
	if ($oferta->Titlu==""){
		...
		$oferta->Titlu.=$cartier->Denumire.($cartier->Denumire!=$subzona->Denumire ? " - ".$subzona->Denumire : "");
	...
	// sincronizare imobiliare.ro
	if (($oferta->ExportImobiliare>0) && (SyncSite::modalitate("Imobiliare") == "automat")) {
	...
	// sincronizare imopedia
	
	if (( $oferta->ExportImopedia > 0 ) && (SyncSite::modalitate("Imopedia") == "automat")) {
		$imopedia = new Imopedia($apartament, $oferta, $subzona, $cartier, $oras);
		
		//echo $oferta->ExportImopedia;
		...
		<div id="oferta" style="overflow:auto; background-color: rgb(255,248,226);">
			<div style="display: inline-block; float: left">
				...
				<fieldset>
					...
					<label class="label">stare</label><select class="standard" id="OfStare" name="OfStare">
						...
						<option value="inchiriat" <?php if ($oferta->Stare=="inchiriat") {echo "selected";}?>>inchiriat</option>
			...
			<div style="display: inline-block; float: left">
				<fieldset>
					...
					<label class="label" id="LabelNumarBalcoane_xaxbxcxs">balcoane</label><select id="NumarBalcoane_xaxbxcxs" name="ApNumarBalcoane" class="standard">
						<option value=''> </option>
						<?php 
							for ($i = 0; $i < 10; $i++) {
								echo "<option value='".$i."'";
					...
					<label class="label" id="LabelNumarTerase_xaxbxcxs">terase</label><select id="NumarTerase_xaxbxcxs" name="ApNumarTerase" class="standard">
						<option value=''> </option>
						<?php 
							for ($i = 0; $i < 10; $i++) {
								echo "<option value='".$i."'";
					...
					<?php echo put_item("sup totala constr.", "SuprafataConstruita", $apartament->SuprafataConstruita, "Ap", "xaxbxcxtxs")?> 
					<?php echo put_item("s balcoane/terase", "SuprafataTerasa", $apartament->SuprafataTerasa, "Ap", "xaxbxcxs")?>
					
					<?php echo put_item("sup teren", "SuprafataCurte", $apartament->SuprafataCurte, "Ap", "xbxcxs")?>
		...
		<div id="promovare" style="overflow:auto;background-color: rgb(255,248,226);">
			<table style="width: 500px; padding-left: 5px;">
				...
				<tr class="row odd" >
					...
					<td style="padding-left: 5px;" ><input type="checkbox" id="OfOfertaWeb" name="OfOfertaWeb"  <?php if (($oferta->OfertaWeb>1)&&($oferta->OfertaWeb!=3)) {echo "checked=\"checked\"";}?> value="1"></input></td>
				...
				<tr class="row even">
					...
					<td style="padding-left: 5px;"><input type="checkbox" id="OfExportImopedia" name="OfExportImopedia"  <?php if (($oferta->ExportImopedia>0)&&($oferta->ExportImopedia!=3)) {echo "checked=\"checked\"";}?> value="1"></input></td>
					<td style="padding-left: 5px;"></td>
					<td class="ui-corner-right" style="padding-left: 5px;"></td>
				</tr>
				<tr class="row odd">
					<td class="ui-corner-left" style="padding-left: 5px;">propertyBook</td>
					<td style="padding-left: 5px;"><input type="checkbox" id="OfExportPB" name="OfExportPB"  <?php if (($oferta->ExportPB>0)&&($oferta->ExportPB!=3)) {echo "checked=\"checked\"";}?> value="1"></input></td>
					<td style="padding-left: 5px;"></td>
					<td class="ui-corner-right" style="padding-left: 5px;"></td>
				</tr>
			</table>
			...
			<div style="display: inline-block; float: left">
				<fieldset>
					...
					<dl>
						...
						<dt><label class="label">c.i.</label><input type="text" id="SerieCI" name="ClSerieCI" style="width: 30px;"  value="<?php echo $client->SerieCI; ?>"></input><input type="text" style="text-align: right; width: 144px;" name="ClNumarCI" value="<?php echo $client->NumarCI; ?>"></input></dt>
