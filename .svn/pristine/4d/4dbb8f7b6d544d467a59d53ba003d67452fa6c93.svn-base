	...
	fwrite($fh, "<property>\n");
		...
		foreach ($fotografii as $foto) {
			...
			fwrite($fh, "<picture-url>");
				fwrite($fh, "http://igor.lanconect.ro/Imob2009/images/{$foto->NumeFisier}");
			fwrite($fh, "</picture-url>\n");
		...
		fwrite($fh, "<agency>\n");
			...
			fwrite($fh, "<agency-norc-username>");
				fwrite($fh, "simsparkman");							// ? trebuie sa mi-l dea igor
			fwrite($fh, "</agency-norc-username>\n");
			...
			fwrite($fh, "<agency-street-address>");
				fwrite($fh, "Calea Calarasi, Nr. 165, Bl. 38, Sc. 2, Ap. 19, Sector 3");
			fwrite($fh, "</agency-street-address>\n");
...
$fisier=".".DS."export".DS."norc.xml";
$fh=fopen($fisier, "w");
fwrite($fh, "<properties>\n");

$sql="SELECT * FROM Oferta WHERE ExportNorc>0";// AND ExportNorc<>3 AND Stare='de actualitate'"; //
// status oferta: 
//					0 - nu se exporta
//					1 - creare
//					2 - update
...
if (!empty($oferte)){
	foreach ($oferte as $oferta) {
		if (( $oferta->ExportNorc <> 3 ) && ( $oferta->Stare == "de actualitate" )){
		...
		//$oferta->ExportNorc=($oferta->ExportNorc==3 ? 0 : 4);
		//if ($oferta->Stare!='de actualitate') {$oferta->ExportNorc=0;}
		$oferta->save();
	}
}
