	...
	if (strlen($q)>2) {
		...
		if (!empty($stradaList))
		{
			...
			foreach ($stradaList as $strada)
			{
				...
				$result.="<tr id=\"StrHint{$strada->id}\" class=\"$class\" onclick=stradaHintSelect('".str_replace(" ","%20",$Denumire)."','divStradaHint{$ord}','{$ord}') onmouseover=\"this.style.cursor='pointer';stradaHintOnOver('StrHint{$strada->id}',1,$i)\" onmouseout=\"stradaHintOnOver('StrHint{$strada->id}',0,$i)\"><td>{$strada->Denumire}</td></tr>";
			}
			$result.="</table>";
		}	
	}
